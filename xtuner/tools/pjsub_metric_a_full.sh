#!/bin/bash
# =============================================================================
# Metric A continue-SFT — FULL run (129677-example train json). Genkai b-batch, 1 node,
# 4x H100, ZeRO-2, full fine-tune (LongNet+projector+LLM) from SlideChat stage-2 weights.
# FULLY SELF-CONTAINED: every path/var hardcoded below; does NOT inherit any login-shell env.
#
# Submit ALONGSIDE pjsub_metric_a_smoke.sh: the smoke (short elapse) backfills/runs first;
# this full job queues in parallel so no wall-clock is lost once the smoke validates the path.
# If the smoke reveals a problem, cancel this job (pjdel <jobid>) before it starts.
#
#   pjsub /home/pj24003162/ku40003404/weihao/00/reg2026_slidechat_dev/xtuner/tools/pjsub_metric_a_full.sh
#
# Twin of pjsub_metric_a_smoke.sh (ONLY differences: elapse / DATA_PATH / WORK_DIR / MASTER_PORT /
# -o name + derived log/echo). elapse=24:00:00 is a margin over the ~8-12h/epoch estimate;
# tighten from the smoke's it/s. (Confirm the live b-batch wall-time cap with `pjshowrsc` first.)
# =============================================================================
...
NGPU=4
LOGDIR=$REG2026/logs
DATA_PATH=$REG2026/data/reg2026/metric_a/sft_metric_a_train.json
STAMP=${PJM_JOBID:-local}                 # PJM runtime var (set by scheduler, not login env)
WORK_DIR=$REG2026/outputs/experiments/exp_metricA_sft_full_${STAMP}
MASTER_PORT=29556
...
echo "FULL Job $STAMP  host=$(hostname)  NGPU=$NGPU  $(date)"
echo "CONFIG=$CONFIG"
